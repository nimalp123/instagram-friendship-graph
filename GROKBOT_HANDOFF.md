...
> My own Instagram export is already processed. Please do **not** repeat `YOUR_HANDLE` in `observations.json`. To expand the graph, start with up to 10 of my reciprocal-follow accounts that I name or approve. For each other account, record usernames actually visible in its `followers` and `following` lists only if both lists are available to my logged-in account or shared by that account owner. Put one object per account in a new JSON file with exactly `account`, `followers`, and `following`. If a list is hidden, omit that account. If scrolling stops early or captured counts differ from badges, include only observed names and report badge counts, captured counts, and scroll status separately. Return the JSON privately without replacing earlier batch files. Do not upload it to GitHub.
